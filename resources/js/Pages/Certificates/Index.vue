    ...
    <AuthenticatedLayout>
        ...
        <div v-if="jobs.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="j in jobs" :key="j.transaction_id" class="bg-white border border-gray-200/80 rounded-2xl p-5 hover:shadow-md hover:border-gray-300 transition-all group">
                ...
                <a :href="route('certificates.download', j.transaction_id)" class="w-full flex items-center justify-center gap-2 h-9 rounded-xl text-xs font-semibold text-gray-700 bg-gray-100 hover:bg-gray-900 hover:text-white border border-gray-200 hover:border-gray-900 transition-all">
