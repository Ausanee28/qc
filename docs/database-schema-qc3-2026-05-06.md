# Database Schema Snapshot (qc3)

- Generated at: 2026-05-06 16:42:03
- Driver: mariadb
- Database: qc3
- Source: live database via `information_schema`
- Table count: 12
...
| `Internal_Users` | 15 |
| `migrations` | 38 |
| `performance_daily_inspector_aggregates` | 21 |
| `report_daily_aggregates` | 48 |
| `report_monthly_aggregates` | 45 |
| `Test_Methods` | 18 |
| `Transaction_Detail` | 192 |
| `Transaction_Header` | 179 |
...
- This file is a live schema snapshot for documentation only; it does not include row data dumps.
- Display-only labels such as `Job 001` and `Result 001` are computed in application code and are not stored as database columns.
