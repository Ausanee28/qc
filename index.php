        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-slate-800">
                    <th class="px-6 py-3">ID</th><th class="px-6 py-3">Line</th><th class="px-6 py-3">DMC</th><th class="px-6 py-3">Equipment</th><th class="px-6 py-3">Sender</th><th class="px-6 py-3">Receiver</th><th class="px-6 py-3">Received</th><th class="px-6 py-3">Status</th><th class="px-6 py-3">Action</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-800/60">
                <?php if (empty($recentItems)): ?>
                    <tr><td colspan="9" class="px-6 py-10 text-center text-slate-600">No transactions yet. <a href="receive_job.php" class="text-indigo-400 hover:underline font-medium">Receive a job</a> to get started.</td></tr>
                <?php else: ?>
                    <?php foreach ($recentItems as $i => $item): ?>
                        <tr class="hover:bg-slate-800/30 transition-all duration-200 table-row-anim" style="animation-delay: <?= 0.7 + ($i * 0.06) ?>s">
                            ...
                            <td class="px-6 py-3">
                                <a href="print_tag.php?id=<?= $item['transaction_id'] ?>" target="_blank" class="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 rounded-lg text-purple-400 text-xs font-bold transition-all" title="Print QR Tag">
