    ...
    <aside class="w-64 bg-slate-900/80 border-r border-slate-800 flex flex-col fixed inset-y-0 left-0 z-30 anim-slide-left">
        <div class="px-6 py-5 border-b border-slate-800">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-sm font-bold text-white leading-tight">QC Lab Tracking</h1>
                    <p class="text-xs text-slate-500">Quality Control</p>
                </div>
            </div>
        </div>
        <nav class="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
            <a href="index.php" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-400 <?= $currentPage === 'index.php' ? 'active' : '' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                Dashboard
            </a>
            <a href="receive_job.php" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-400 <?= $currentPage === 'receive_job.php' ? 'active' : '' ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                Receive Job
            </a>
            <a href="execute_test.php" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-400 <?= $currentPage === 'execute_test.php' ? 'active' : '' ?>">
            ...
            <a href="report.php" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-slate-400 <?= $currentPage === 'report.php' ? 'active' : '' ?>">
        ...
        <div class="px-4 py-4 border-t border-slate-800 anim-fade-in delay-3">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-sm font-bold">
                    <?= strtoupper(substr($_SESSION['name'] ?? 'U', 0, 1)) ?>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-white truncate"><?= htmlspecialchars($_SESSION['name'] ?? '') ?></p>
                    <p class="text-xs text-slate-500 truncate"><?= htmlspecialchars($_SESSION['role'] ?? '') ?></p>
                </div>
    ...
    <main class="flex-1 ml-64">
        <header class="sticky top-0 z-20 bg-slate-950/80 backdrop-blur-lg border-b border-slate-800 px-8 py-4 anim-fade-in">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold text-white"><?= htmlspecialchars($pageTitle ?? 'Dashboard') ?></h2>
                    <p class="text-sm text-slate-500 mt-0.5"><?= htmlspecialchars($pageSubtitle ?? '') ?></p>
                </div>
                <div class="text-sm text-slate-500"><?= date('l, d M Y') ?></div>
            </div>
        </header>
